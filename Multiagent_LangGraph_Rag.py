# Multi-Agent RAG System - Anupama Sharma
...
class MultiAgentRAGSystem:
    """
    A comprehensive multi-agent system built by Anupama Sharma for handling
    ...
    def _setup_rag_chain(self):
        ...
        try:
            ...
            You are Annu's AI assistant. Use the available context to answer the question. 
    ...
    def _setup_agents(self):
        ...
        self.search_agent = self._create_agent(
            self.llm,
            [self.tavily_tool],
            "You are Annu's research assistant who can search for up-to-date info using the search engine.",
        )
        
        self.research_agent = self._create_agent(
            self.llm,
            [self.retrieve_information],
            "You are Annu's research assistant who can provide specific information on the provided paper: 'Extending Llama-3's Context Ten-Fold Overnight'",
        )
        ...
        self.doc_writer_agent = self._create_agent(
            ...
            "You are Annu's expert writing assistant for research documents.",
        )
        
        self.note_taking_agent = self._create_agent(
            self.llm,
            [self.doc_tools[0], self.doc_tools[1]],  # create_outline, read_document
            "You are Annu's expert senior researcher tasked with writing blog outlines and taking notes to craft perfect technical content.",
        )
    ...
    def _build_graphs(self):
        ...
        supervisor_node = self._create_team_supervisor(
            self.llm,
            "You are Annu's main supervisor tasked with managing a conversation between the"
...
def main():
    ...
    print("Multi-Agent RAG System by Anupama Sharma")
    print("=" * 50)
